...
public class UserServiceImpl implements UserInterface {
	...
	public User addUser(User user) {
		...
		{
		String insert_query="insert into user_details(user_name,user_password,user_phone,"
				+ "user_dob,user_address,user_type,user_gender) values(?,?,?,?,?,?,?)";
   ...
   ps.setString(7, user.getGender());
	...
	public User getLogin(String userName, String password) {
		...
		try(Connection con=DBUtil.getConnect())
		{
			String loginQuery="select * from user_details where "
       ...
       if(rs.next())
       {
    	 ...
    	 //to construct the user object we need to convert sql date into util
			...
			User user = new User(uid,un,pw,utype,uadd,ph,d1,gen);
